...
typedef SWIFT_ENUM(NSInteger, AMSLogType, open) {
  AMSLogTypeUnknown = -1,
  AMSLogTypeSeosCore = 0,
  AMSLogTypeStatistics = 1,
  AMSLogTypeBeacon = 2,
  AMSLogTypeSeosCache = 3,
  AMSLogTypeSeosApdus = 4,
  AMSLogTypeSeosApdusClear = 5,
  AMSLogTypeSeosApi = 6,
  AMSLogTypeSeosAccessApi = 7,
  AMSLogTypeBleCommunication = 8,
  AMSLogTypeBleConnection = 9,
  AMSLogTypeLifecycle = 10,
  AMSLogTypeKeychain = 11,
  AMSLogTypeNetwork = 12,
  AMSLogTypePersonalization = 13,
  AMSLogTypeApi = 14,
  AMSLogTypeOpening = 15,
  AMSLogTypeFileSystem = 16,
  AMSLogTypeErrorHandler = 17,
  AMSLogTypeReader = 18,
  AMSLogTypeRssi = 19,
  AMSLogTypeSeosSession = 20,
  AMSLogTypeNfc = 21,
  AMSLogTypeSecureChannelOpen = 22,
  AMSLogTypeTaskchain = 23,
  AMSLogTypeRotation = 24,
  AMSLogTypeApp = 25,
};

@class NSString;
SWIFT_CLASS("_TtC17SeosMobileKeysSDK9AMSLogger")
@interface AMSLogger : NSObject
/// <code>isEnabled</code> returns true if Logger is enabled, else false
...
+ (void)startLogger;
+ (void)stopLogger;
+ (void)exportWithSdkVersion:(NSString * _Nonnull)sdkVersion completion:(void (^ _Nonnull)(NSArray<NSString *> * _Nonnull))completion;
+ (void)log:(NSString * _Nonnull)message category:(enum AMSLogType)category;
+ (void)warn:(NSString * _Nonnull)message;
+ (void)debug:(NSString * _Nonnull)message category:(enum AMSLogType)category;
+ (void)info:(NSString * _Nonnull)message category:(enum AMSLogType)category;
+ (void)error:(NSString * _Nonnull)message category:(enum AMSLogType)category;
@end
...
SWIFT_EXTERN void integrity_d(void) SWIFT_NOEXCEPT;

SWIFT_EXTERN void integrity_j(void) SWIFT_NOEXCEPT;
