...
typedef NS_ENUM(NSInteger, MobileKeysOpeningStatusType) {
    ...
    /**
     * Reader communication failed in bidirectional mode
     */
    MobileKeysOpeningStatusTypeBiDirectionalFailure = 17
};
...
/**
 * An optional scan response data (tag 2 and data)
 */
@property(nonatomic, strong) NSData *optionalScanResponseData;

/**
 * Serial number data from tag 2
 */
@property(nonatomic, strong) NSData *serialNumberData;

/**
 * App specific data fom tag 3
 */
@property(nonatomic, strong) NSData *appSpecificData;

/**
 * Access to currently connected BLE periphera (CBPeripheral)
 */
...
/**
 * Returns RSSI limit for specific opening type.
 * @param type - Opening type to get RSSI limit for.
*/
- (NSInteger)getRssiLimitForOpeningType:(MobileKeysOpeningType)type;
