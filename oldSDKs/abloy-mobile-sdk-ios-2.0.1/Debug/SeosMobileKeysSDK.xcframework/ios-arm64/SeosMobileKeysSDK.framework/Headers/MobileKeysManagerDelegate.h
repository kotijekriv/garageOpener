...
/**
 * Callback that, if implemented, will be invoked when a connected device wishes to open an encrypted
 * bi-directional communication channel with the endpoint.
 * @param channel - The channel through which the connected reader or other device will communicate
 * directly with the application. Note that the channel is not retained by the SDK, so hold a strong reference
 * to it as long as it is needed.
 * @note since version 8.0.0
 */
@optional
- (void)mobileKeysDidOpenSecureChannel:(SeosSecureChannel *)channel;

/**
 * Callback that, if implemented, will be invoked when a connected device exits an encrypted
 * bi-directional communication channel with the endpoint.
 * @note since version 8.0.0
 */
@optional
- (void)mobileKeysDidExitSecureChannel;

/**
 * Callback for when the reader gets inactive and removed from readers repository
 * @note since version 8.3.3
 */
@optional
