...
__attribute__(( visibility("default") )) extern const NSString *MobileKeysOptionApplicationId;
__attribute__(( visibility("default") )) extern const NSString *MobileKeysOptionVersion;
__attribute__(( visibility("default") )) extern const NSString *MobileKeysOptionBeaconUUID;
__attribute__(( visibility("default") )) extern const NSString *MobileKeysOptionTSMPublicCert;
__attribute__(( visibility("default") )) extern const NSString *MobileKeysOptionTSMBaseURL;
__attribute__(( visibility("default") )) extern const NSString *MobileKeysOptionMotionManager;
__attribute__(( visibility("default") )) extern const NSString *MobileKeysOptionCustomEventValue;
__attribute__(( visibility("default") )) extern const NSString *MobileKeysOptionCustomSetupAkeKey;
__attribute__(( visibility("default") )) extern const NSString *MobileKeysOptionCustomAttributeField;
__attribute__(( visibility("default") )) extern const NSString *MobileKeysOptionConnectingToReaderStopsScanning;
...
typedef NS_ENUM(NSInteger, MobileKeysScanMode) {
    /**
     * Enable iBeacon Monitoring while scanning for readers
     */
            MobileKeysScanModeOptimizePerformance = 0,
    /**
     * Do not enable iBeacon Monitoring while scanning for readers
     */
...
/**
 * Returns data for the specified `-[SeosTag]` on key with specified object identifier (OID). The OIDString is the keyId property of the `MobileKeysKey`.
 *
 * @param oidString - OIDs of keys to get data from.
 * @param tag - `SeosTag` to get data from.
 * @param error - Error object. Library will set this if an error occurs
 * @version 2.0.0
 */
- (NSData *)dataForKeyOid:(NSString *)oidString tag:(SeosTag *)tag error:(NSError **)error;
 ...
 * @param error - Error object. Library will set this if an error occurs.
 */
- (void)putData:(NSData *)data keyOid:(NSString *)oidString tag:(SeosTag *)tag error:(NSError **)error;
 ...
 * Seos.
 * The current implementation of theSeosProvider suffers from a few limitations:
 ...
 */
- (SeosProvider *)defaultMobileKeysSeosProvider;

/**
* Use this method to take over the responsibility for the connected bluetooth peripheral. Handover of the peripheral to a new delegate (not the SDK). When the SDK has connected to the peripheral, the resposibity is handed over the the new delegate the can change services and characteristics and then recieve the callbacks from the connected peripheral. The delegate is required to implement the CBPeripheralDelegate protocol
* @return - YES on success, NO on failure
* @param delegate - the delegate to hand over responsibility to, that implemts the CBPeripheralDelegate protocol
*/
- (BOOL)handoverPeripheralToDelegate: (id <CBPeripheralDelegate> _Nullable) delegate;

/**
* Use this method to cancel a reader connection. This method overrides the SDK and sends a cancel peripheral connection to the specified reader.
* @param reader - the reader which is to be disconnected
*/
